...
int
ukmapcopy(pagetable_t old, pagetable_t new, uint64 start, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(start); i < start+sz; i += PGSIZE){
    if((pte = walk(old, i, 0)) == 0)
      panic("ukmapcopy: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("ukmapcopy: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    flags &= ~PTE_U;
...
int
ukmapdel(pagetable_t old, pagetable_t new, uint64 start, uint64 end){
  start = PGROUNDUP(start);
  end = PGROUNDUP(end);
  uint64 num = (end-start)/PGSIZE;
  uvmunmap(new, start, num, 0);
  return 0;
}